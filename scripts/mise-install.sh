...
Usage: $this [-b bindir] [-d] [-q] [-n] [tag]
  ...
  -n turns on dry run mode
   ...
   If tag is missing, then v2026.6.10 will be used.
...
resolve_asset_filename() {
  ...
  if [ "${UNAME_OS}" = 'darwin' ] && true
  then
    ASSET_FILENAME="mise-${TAG}-${OS}-${ARCH}${EXT}"
  fi
  if [ "${UNAME_OS}" = 'windows' ] && true
  then
    EXT='.zip' ASSET_FILENAME="mise-${TAG}-${OS}-${ARCH}${EXT}"
  fi
  if [ -z "${ASSET_FILENAME}" ]; then
    ASSET_FILENAME="mise-${TAG}-${OS}-${ARCH}-musl${EXT}"
  fi
}
...
execute() {
  STRIP_COMPONENTS=0
  CHECKSUM_FILENAME="SHASUMS256.txt"
